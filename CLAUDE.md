...
- **Every multi-step write happens inside a transaction that rolls back on failure.** Follow the existing pattern: `conn.execute("BEGIN")`, do the writes, `conn.commit()` in the success path, `conn.rollback()` + re-raise in an `except Exception:` block. See `db/orders.py::record_order`, `db/production.py::run_production_batch`, `db/distributions.py::record_profit_distribution` for examples. A function that only does a single `UPDATE`/`INSERT` (e.g. `deactivate_product`) doesn't need the explicit `BEGIN`/`rollback` dance — but anything touching more than one table, or validating before writing, does.
...
- **Customer-facing documents show only what the customer pays.** `pdf/invoice.py` (and any future customer-facing document) must never display internal costs — no postage paid, packaging cost, transaction fee amount as a cost, or product `unit_cost`/COGS. It may show what the customer is charged (item prices, discounts, shipping charged to the customer, the transaction-fee line as a deduction from what they receive if that's a customer-visible adjustment) but never the shop's internal cost basis.
